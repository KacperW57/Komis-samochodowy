...
export const carsSlice = createSlice({
  ...
  reducers: {
    ...
    ratingPlus(state, action) {
      const carsId = action.payload;
      const carRating = state.find((array) => array.id === carsId);
      if (carRating) {
        carRating.rating++;
      }
    },
    ratingMinus(state, action) {
      const carsId = action.payload;
      const carRating = state.find((array) => array.id === carsId);
      if (carRating) {
        carRating.rating--;
...
export const { addCar, removeCar, ratingPlus, ratingMinus } = carsSlice.actions;
